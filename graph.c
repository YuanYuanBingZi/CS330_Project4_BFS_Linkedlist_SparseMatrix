...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    //create all the vertix and add them at the first position of the each row of the adj_list
    for(int i = 0; i < rows;i++){
        for(int j = 0; j < rows; j++){
            if (adj_mat[i][j] == 1){
                adj_node_t* vertex = create_node(j);
                add_node(myList, i, vertex);
            }
        }
    }
         ...
         int* color, int* distance, int* parent)
{
    ...
    while(!isEmpty(q)){
        ...
        int temp[rows];
        adj_node_t* next = list[current_vertex];
        int index = 0;
        while(next != NULL) {
            temp[index] = next->vid;
            index++;
        ...
        for(int i = 0; i < index; i++){
            if (color[temp[i]] == 0){
                color[temp[i]] = 1;
                parent[temp[i]] = current_vertex;
                distance[temp[i]] = distance[current_vertex] + 1;
                enqueue(q,temp[i]);
            }
        }
